...
export const TasksCard = () => {
    const [expandedSections, setExpandedSections] = useState<TTaskExpandedSections>({
        inProgress: true,
        todo: false,
        upcoming: false
    });
    ...
    const renderTaskRow = (task: TTask) => (
    ...
    const renderSection = (
        ...
        return (
            <div className="mb-1">
                <div 
                    className="flex items-center gap-2 ml-5 mb-3 cursor-pointer hover:bg-accent rounded"
                ...
                >
                    ...
                    <span className="text-sm text-foreground">• {sectionTasks.length} task{sectionTasks.length !== 1 ? 's' : ''}</span>
                </div>
                        ...
                        <Table className="min-w-full">
                            <TableHeader>
                                <TableRow className="border-b border-border">
                                    ...
                                    <TableHead className="text-muted-foreground font-normal text-xs py-2 px-3 text-right">Due date</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                ...
                {isExpanded && (
                    <Button variant="ghost" className="text-foreground px-4 h-auto text-sm mb-2 ml-2 hover:bg-transparent hover:text-foreground">
                        + Add task
                    </Button>
                )}
            </div>
