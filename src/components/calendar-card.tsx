    ...
    return (
        <div className="bg-card p-4 sm:p-6 rounded-2xl border border-border">
            <div className="flex items-center mb-6">
                ...
                <Button
                    variant="ghost"
                    className="flex items-center gap-1 hover:bg-accent px-3 py-2 rounded-lg transition-colors"
                >
            ...
            <div className="flex items-center gap-2 mb-6">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5 sm:gap-4 flex-1">
                    {dates.map((date: TDateItem, index: number) => (
                        <div
                            key={index}
                            className={`flex flex-col items-center justify-center py-2 sm:py-2 sm:px-1  rounded-lg sm:rounded-xl cursor-pointer transition-all duration-200 lg:max-w-14 ${index > 3 ? "hidden sm:flex" : "flex"
                                } ${date.active
                                    ? "bg-gradient-to-br from-prodify-primary to-accent-purple text-white shadow-lg transform scale-105"
                                    : "hover:bg-accent hover:shadow-md"
                                }`}
                        >
                            <span className={`text-[10px] sm:text-xs md:text-sm font-medium ${date.active ? "text-white" : "text-muted-foreground"
                                }`}>
                                {date.day}
                            </span>
